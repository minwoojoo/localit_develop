...
class TravelerMatchRequestsScreen extends StatelessWidget {
  const TravelerMatchRequestsScreen({super.key});
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final requesterNickname = data['requester_nickname'] ?? '알 수 없음';
  ...
  Future<void> _createChatRoom(BuildContext context, String requestId) async {
    try {
      ...
      final requesterId = requestData['requester_id']; // 로컬인 ID
      final receiverId = requestData['receiver_id']; // 여행자 ID

      // 이미 채팅방이 존재하는지 확인
      final existingChatRoom = await FirebaseFirestore.instance
          .collection('chat_rooms')
          .where('traveler_id', isEqualTo: receiverId)
          .where('local_id', isEqualTo: requesterId)
          .get();
      ...
      await FirebaseFirestore.instance.collection('chat_rooms').add({
        'traveler_id': receiverId, // 여행자 ID
        'local_id': requesterId, // 로컬인 ID
        'related_match_id': requestId, // 매칭 요청 ID
        'created_at': FieldValue.serverTimestamp(),
      });

      print('채팅방이 성공적으로 생성되었습니다.');
      print('traveler_id: $receiverId');
      print('local_id: $requesterId');
